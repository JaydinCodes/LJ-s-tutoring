...
test('architecture docs identify the active app and Supabase-first ownership model', () => {
  ...
  assert.match(architecture, /`student-app\/` and `legacy\/static\/` are inactive reference material/);
});

test('architecture docs keep Fastify transitional and document required diagrams', () => {
  const architecture = read('docs/architecture/ARCHITECTURE.md');

  assert.match(architecture, /`lms-api\/` is transitional and feature-specific, not the browser auth authority/);
  assert.match(architecture, /Browser-protected routes should use Supabase session\/profile state/);
