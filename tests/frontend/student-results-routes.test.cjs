...
test('results overview is premium summary-only and calculated from released results', () => {
  ...
  for (const label of ['Overall Average', 'Best Subject', 'Weakest Topic', 'Latest Mark', 'Mark Trend', 'Consistency Score']) {
    assert.ok(source.includes(`label="${label}"`), `${label} metric must be present`);
  }
  ...
  assert.ok(source.includes('function PremiumResultMetric'), 'overview must use premium metric styling');
...
test('result detail and subject routes expose private learner results without classmate data', () => {
  ...
  assert.ok(source.includes('No classmate names or individual marks are shown'), 'class analytics copy must reinforce anonymity');
