
import {
  apiFetch,
  clearChildren,
  loadJson,
  renderEmpty,
  renderError,
  renderList,
  renderLoading,
} from '/assets/common.js';
import { track } from '/assets/analytics-module.js';

const LIST_ID = 'tutorReportsList';

function toText(value, fallback = '') {
  if (value === null || value === undefined) {return fallback;}
  return String(value);
}

function toDateOnly(value) {
  if (!value) {return '';}
  const str = String(value);
  return str.length >= 10 ? str.slice(0, 10) : str;
}

function formatCreatedAt(value) {
  if (!value) {return '';}
  const d = new Date(value);
  if (Number.isNaN(d.getTime())) {return toText(value);}
  return d.toLocaleString();
}

function makeDetailLine(label, value) {
  const line = document.createElement('div');
  line.className = 'report-detail-line';
  const labelEl = document.createElement('strong');
  labelEl.textContent = `${label}: `;
  const valueEl = document.createElement('span');
  valueEl.textContent = toText(value, '—');
  line.append(labelEl, valueEl);
  return line;
}

function renderDetail(detail, report) {
  clearChildren(detail);
  detail.hidden = false;
  const payload = report && report.payload ? report.payload : {};
  detail.append(
    makeDetailLine('Week', `${toDateOnly(report.weekStart)} → ${toDateOnly(report.weekEnd)}`),
    makeDetailLine('Generated', formatCreatedAt(report.createdAt)),
    makeDetailLine('Sessions attended', payload.sessionsAttended ?? 0),
    makeDetailLine('Minutes studied', payload.minutesStudied ?? 0),
  );
  if (payload.summary) {
    const p = document.createElement('p');
    p.className = 'report-detail-note';
    p.textContent = String(payload.summary);
    detail.appendChild(p);
  }
}

function renderReportRow(item) {
  const wrapper = document.createElement('div');
  wrapper.className = 'list-item report-item';

  const title = document.createElement('strong');
  title.textContent = toText(item.student_name, 'Student report');
  const range = document.createElement('div');
  range.textContent = `${toDateOnly(item.week_start)} → ${toDateOnly(item.week_end)}`;
  const created = document.createElement('div');
  created.textContent = `Created: ${formatCreatedAt(item.created_at)}`;

  const detail = document.createElement('div');
  detail.className = 'report-detail';
  detail.hidden = true;

  const actions = document.createElement('div');
  actions.className = 'report-actions';

  const viewBtn = document.createElement('button');
  viewBtn.type = 'button';
  viewBtn.className = 'button secondary';
  viewBtn.textContent = 'View details';
  viewBtn.addEventListener('click', async () => {
    if (!detail.hidden) {
      detail.hidden = true;
      viewBtn.textContent = 'View details';
      return;
    }
    viewBtn.disabled = true;
    viewBtn.textContent = 'Loading…';
    try {
      const data = await loadJson(`/reports/${encodeURIComponent(item.id)}`);
      if (data && data.report) {
        renderDetail(detail, data.report);
        track('report.viewed', { reportId: item.id });
      } else {
        renderError(detail, 'Could not load report.');
        detail.hidden = false;
      }
      viewBtn.textContent = 'Hide details';
    } catch (_err) {
      renderError(detail, 'Could not load report.');
      detail.hidden = false;
      viewBtn.textContent = 'View details';
    } finally {
      viewBtn.disabled = false;
    }
  });

  const generateBtn = document.createElement('button');
  generateBtn.type = 'button';
  generateBtn.className = 'button';
  generateBtn.textContent = 'Regenerate';
  generateBtn.addEventListener('click', async () => {
    if (!item.student_id) {return;}
    generateBtn.disabled = true;
    const prev = generateBtn.textContent;
    generateBtn.textContent = 'Generating…';
    try {
      const res = await apiFetch('/reports/generate', {
        method: 'POST',
        body: { studentId: item.student_id },
      });
      if (res.ok) {
        const body = await res.json();
        track('report.generated', { reportId: body?.report?.id, studentId: item.student_id });
        await loadReports();
      }
    } catch (_err) {
      /* fail-safe — keep UI responsive */
    } finally {
      generateBtn.textContent = prev;
      generateBtn.disabled = false;
    }
  });

  actions.append(viewBtn, generateBtn);
  wrapper.append(title, range, created, actions, detail);
  return wrapper;
}

async function loadReports() {
  const target = document.getElementById(LIST_ID);
  if (!target) {return;}
  renderLoading(target, 'Loading reports…');
  try {
    const data = await loadJson('/tutor/reports');
    const items = data.items || [];
    if (!items.length) {
      renderEmpty(target, 'No reports yet for your linked students.');
      return;
    }
    renderList(target, items, (item) => renderReportRow(item));
  } catch (_err) {
    renderError(target, 'Could not load reports. Please try again.');
  }
}

await loadReports();
