...
was needed anywhere (§5). Per-domain "Explicitly deferred" callouts further down
this document (in §2, §4A–§4E) describe the state **at the time each domain's
schema landed** and are now stale on frontend-repoint and Fastify-retirement
claims specifically — §6's sequencing list is the up-to-date summary; trust that
over the inline deferred-items lists if they conflict.
...
**✅ Backfill decision CLOSED (owner confirmed, 2026-07-24): no backfill needed, anywhere, including finance.** The owner confirmed no real money or session history was ever recorded through the Prisma/Fastify system. Verified directly against production: every operational table (`sessions`, `session_history`, `invoices`, `invoice_lines`, `pay_periods`, `adjustments`, `weekly_reports`, `student_notifications`, `tutor_applications`, `tutor_documents`, `baseline_assessments`, `learning_goals`, `student_exam_events`, `volunteer_events`, `volunteer_logs`, `student_score_snapshots`, `career_progress_snapshots`, `audit_log`, `privacy_requests`) is at 0 rows; only `profiles`/`students`/`tutors` have a handful of rows, all from this migration's own functional-verification testing, not real usage. Also confirmed no `_prisma_migrations` table exists in the database at all — Prisma's own migrations were never actually applied here. This means every "explicitly deferred: real data backfill" note elsewhere in this document is now resolved as **not applicable, nothing to migrate** — clean cutover everywhere.
...
2. **Sessions & attendance** — the linchpin; unblocks everything operational. ✅ **Supabase schema + RLS + RPC layer landed.** No real-data backfill needed (§5). ✅ **Frontend repoint done** for reads, report save/submit, approve/reject (`tutorOperationsRepository.ts`, `adminOperationsRepository.ts`). ⚠️ **Known gap: nothing creates a session.** No UI anywhere calls `create_session`/`update_session` — confirmed by the owner (2026-07-24) as genuinely missed, not a deliberate cut. Real follow-up work, not urgent at pilot scale but blocking end-to-end use of the pipeline.
3. **Finance / payroll** — depends on sessions. ✅ **Supabase schema + RLS + payroll RPC layer landed**, frontend repointed (`adminPayrollRepository.ts`/`AdminPayrollRoute.tsx`). No real-data backfill needed (§5). Still open: invoice PDF/HTML rendering (no UI anywhere renders one) — the old Fastify payroll-CSV/audit-CSV export job types were retired, not ported, when `lms-api` was decommissioned (§6 step 7); rebuild as a Supabase RPC/Edge Function if still wanted.
4. **Weekly reports + notifications** — depend on sessions. ✅ **Supabase schema + RLS + RPC layer landed**, weekly reports frontend repointed (`studentReportsRepository.ts`/`tutorOperationsRepository.ts`). No real-data backfill needed (§5). ⚠️ **Student notifications have zero frontend surface** — `student_notifications`/`mark_notification_read`/`mark_all_notifications_read` are never read anywhere in `src/`, even though the session RPCs dispatch real notification rows now. Deliberately deferred UI work (see the top-level note on tutor-onboarding/academic-extras below).
5. **Growth monitoring / risk** (`student_score_snapshots`, `career_progress_snapshots`) — ✅ **Supabase schema + RLS + RPC layer landed**, tutor-side read repointed (`loadTutorRiskScores`). No real-data backfill needed (§5). ⚠️ **Nothing ever recomputes a score** — `recompute_student_risk_snapshot`/`recompute_career_progress_snapshot` have no caller anywhere (no cron, no Edge Function, no frontend trigger), and there is no student-facing score view at all. Real follow-up work: wire a scheduled Edge Function or pg_cron job.
6. **Tutor onboarding** — ✅ **Supabase schema + RLS + RPC layer + Storage bucket landed.** No real-data backfill needed (§5). **Frontend repoint deliberately deferred** — zero existing frontend to repoint against; treated as new-UI backlog work, not part of finishing the migration (owner instruction: "leave the new front end when we get to that place").
6a. **Academic extras + volunteering** (`baseline_assessments`, `learning_goals`, `student_exam_events`, `volunteer_events`, `volunteer_logs`) — ✅ **Supabase schema + RLS + RPC layer landed.** No real-data backfill needed (§5). **Frontend repoint deliberately deferred**, same reasoning as tutor onboarding — zero existing frontend, new-UI backlog.
7. **Retire the Fastify stack** (§3B) and decommission `lms-api`. ✅ **DONE (2026-07-24).** Community suite (§3D) was subsequently un-cut and shipped natively on Supabase (see `docs/supabase/schema.sql`'s Community section) before this step closed — its original "CUT for now" framing no longer applies. `legacy/static/` (unreachable in production) + `tests/e2e-web/` + the `playwright-e2e`/`contracts`/`e2e`/`unit-tests` CI jobs were retired first. `lms-api/` itself (Fastify HTTP API, Prisma ORM, ~78 source files, ~41 tests) is now fully deleted, along with `docker-compose.yml`/`docker-compose.prod.yml`/`docker-compose.gateway.yml` + `ops/gateway/` (existed solely to run it), `deploy-api.yml` (deployed a DO app component that no longer exists), and root `package.json`'s `*-api`/`migrate`/`db:create:local` scripts. The dead `src/lib/api/client.ts` HTTP layer (zero remaining callers) is deleted too.
   - **Ops-script fate (owner decision, 2026-07-24):** `lms-api`'s retention-cleanup/restore-verify/DB-maintenance scripts and their 3 scheduled workflows (`retention-cleanup.yml`, `dr-restore-verify.yml`, `db-maintenance.yml`) were retired outright, not ported forward — investigation found they checked **stale, wrong tables** (`tutor_profiles`/`users`, not the live `tutors`/`profiles`), so porting them would have preserved broken tooling that just doesn't fail loudly. See the retention/DR gap note below.
   - ⚠️ **Real, flagged gap: no working retention/DR coverage for this migration's own tables.** The existing Supabase-native `run_retention_cleanup()` RPC only covers `assignment_submissions`/`student_progress`/`payments`/`tutor_payments`/`audit_log` — it does not touch `sessions`/`invoices`/`invoice_lines`/`pay_periods`/`adjustments` at all. Not urgent (§5: every one of those tables is at 0 rows today), but a real compliance gap for when real usage starts. Needs proper Supabase-native design (pg_cron + a correctly-scoped RPC), not a resurrected copy of the old script.
