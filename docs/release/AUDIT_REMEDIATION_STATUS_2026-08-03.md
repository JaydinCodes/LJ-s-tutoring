...
| Phase 2 web quality | Media optimization, constrained hero-video behavior, route splitting/budgets, accessibility semantics/modal focus, React not-found UI, CSP/form/Sentry alignment, and public copy cleanup are represented in code/tests. | DigitalOcean static-site catch-all cannot currently provide a true unknown-route `404` or arbitrary HSTS/security response headers through this app spec. A Cloudflare proxy is the decided fix for the response-header half (see `docs/deployment/CLOUDFLARE_SECURITY_HEADERS.md`); it requires a DNS/dashboard change outside this repository and is not yet applied. The HTTP 200 catch-all is unaddressed by that plan. | React marks not-found content, but HTTP 200 catch-all and response-header limitations remain documented hosting risks until the Cloudflare setup is actually applied. |
