...
with revision(item_code, activity_type, cognitive_level, representation, difficulty, prompt, answer_config, solution, marks, primary_skill_code, material_change_note) as (
  values
    ...
    ('Q.G9.DOTS.02','diagnostic','routine','symbolic',2,'Expand: (x - 5)(x + 5).','{"accepted_answers":["x^2 - 25"]}'::jsonb,'The middle terms cancel: x^2 + 5x - 5x - 25 = x^2 - 25.',2::numeric,'G9.ALG.EXPAND.BINOMIAL','Second-pass review: retained as an expansion item and removed the unrelated Difference of Two Squares supporting-skill link.'),
    ...
    ('Q.G9.DIAG.07','diagnostic','routine','symbolic',3,'Factorise: x^2 - 25.','{"accepted_answers":["(x - 5)(x + 5)"]}'::jsonb,'Recognise x^2 - 25 as x^2 - 5^2 and use a?? - b?? = (a - b)(a + b): (x - 5)(x + 5).',2::numeric,'G9.ALG.FACTOR.DOTS','Second-pass review: cognitive level corrected to routine.'),
    ('Q.G9.DOTS.03','worked_example','routine','symbolic',2,'Factorise: x^2 - 25.','{"accepted_answers":["(x - 5)(x + 5)"]}'::jsonb,'x^2 - 25 = x^2 - 5^2 = (x - 5)(x + 5).',2::numeric,'G9.ALG.FACTOR.DOTS','Second-pass review: corrected hint values to match x^2 - 25 and its square root 5.'),
    ...
    ('Q.G9.DOTS.09','error_analysis','complex','symbolic',3,'A learner says x^2 - 64 = (x - 8)??. Explain and correct the factorisation.','{"accepted_answers":["(x - 8)(x + 8)"],"response_format":"explanation_and_factorisation","required_response_parts":[{"part_key":"explanation","accepted_concepts":["(x - 8)?? = x^2 - 16x + 64","not x^2 - 64"]},{"part_key":"correct_factorisation","accepted_answers":["(x - 8)(x + 8)"]}]}'::jsonb,'(x - 8)?? expands to x^2 - 16x + 64, not x^2 - 64. Since 64 = 8??, x^2 - 64 = (x - 8)(x + 8).',3::numeric,'G9.ALG.FACTOR.DOTS','Second-pass review: answer configuration now requires both an explanation and the corrected factorisation.'),
...
with hint_seed(item_code, hint_level, prompt) as (
  values
    ...
    ('Q.G9.DOTS.03',1::smallint,'Look at both terms. Are they perfect squares?'), ('Q.G9.DOTS.03',2::smallint,'Recall: a?? - b?? = (a - b)(a + b).'), ('Q.G9.DOTS.03',3::smallint,'x^2 = x^2 and 25 = 5^2.'), ('Q.G9.DOTS.03',4::smallint,'Complete: (x - 5)(x + ___).'), ('Q.G9.DOTS.03',5::smallint,'x^2 - 25 = (x - 5)(x + 5).'),
    ('Q.G9.DOTS.04',1::smallint,'First express 49 as a perfect square.'), ('Q.G9.DOTS.04',2::smallint,'Use the difference-of-two-squares structure.'), ('Q.G9.DOTS.04',3::smallint,'49 = 7^2.'), ('Q.G9.DOTS.04',4::smallint,'Complete: (y - 7)(y + ___).'), ('Q.G9.DOTS.04',5::smallint,'y^2 - 49 = (y - 7)(y + 7).'),
    ('Q.G9.DOTS.09',1::smallint,'A difference of squares is not a binomial square.'), ('Q.G9.DOTS.09',2::smallint,'Compare a?? - b?? with (a - b)??.'), ('Q.G9.DOTS.09',3::smallint,'64 = 8^2.'), ('Q.G9.DOTS.09',4::smallint,'Complete: (x - 8)(x + ___).'), ('Q.G9.DOTS.09',5::smallint,'x^2 - 64 = (x - 8)(x + 8).')
)
