-- SEC-01: secure destructive retention cleanup.
-- Fail closed first.
revoke all on function public.run_retention_cleanup(boolean)
from public, anon, authenticated, service_role;

-- Shared destructive implementation is kept outside the exposed public schema.
...
begin
  ...
  return jsonb_build_object(
    ...
    'files_removed',
      case when p_apply then v_files else null end
  );
end;
$$;

-- Nobody calls the destructive worker directly.
...
-- ADMIN RPC
-- ============================================================
-- Preserve the existing frontend RPC signature.
-- is_platform_admin() already requires:
--   role = admin
--   aal = aal2
...
grant execute on function public.run_retention_cleanup(boolean)
to authenticated;
...
-- SCHEDULER RPC
-- ============================================================
-- Separate path for scheduled server-side execution.
-- Always applies cleanup; there is no caller-controlled boolean.
...
grant execute on function public.run_retention_cleanup_scheduled()
to service_role;


comment on function public.run_retention_cleanup(boolean) is
  'Platform-admin-only retention dry-run/apply RPC. Requires AAL2.';

comment on function public.run_retention_cleanup_scheduled() is
  'Service-role-only scheduled retention apply RPC.';