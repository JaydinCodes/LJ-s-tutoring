-- A single assignment has one grade. Individual learner targets must therefore
-- be active allocations for the tutor AND belong to that same grade.
...
begin
  ...
  if not public.is_platform_admin() and (v_tutor_id is null or v_assignment.created_by <> v_profile_id) then
  ...
  if exists (
    select 1 from unnest(v_student_ids) target_id
    join public.students s on s.id = target_id
    where s.grade is distinct from v_assignment.grade
  ) then raise exception 'assignment_target_grade_mismatch' using errcode = '22023'; end if;
